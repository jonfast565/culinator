...
pub(crate) fn render(recipe: &Recipe, options: &RecipeExportOptions) -> Result<Vec<u8>, ApplicationError> {
    ...
    writer.start_file("mimetype",SimpleFileOptions::default().compression_method(CompressionMethod::Stored)).map_err(err)?; writer.write_all(b"application/epub+zip").map_err(err)?;
    ...
    writer.start_file("OEBPS/nutrition-facts.svg",compressed).map_err(err)?;writer.write_all(label::render(&options.nutrition).as_bytes()).map_err(err)?;
    writer.finish().map(|c|c.into_inner()).map_err(err)
}
fn err<E:std::fmt::Display>(e:E)->ApplicationError{ApplicationError::Internal(e.to_string())}
fn escape(v:&str)->String{v.replace('&',"&amp;").replace('<',"&lt;").replace('>',"&gt;").replace('"',"&quot;")}
