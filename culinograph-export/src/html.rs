...
pub(crate) fn render(recipe: &Recipe, options: &RecipeExportOptions, label_svg: &str) -> Result<String, ApplicationError> {
    let ingredients = recipe.resources.iter().filter(|r| r.kind == ResourceKind::Ingredient).map(|r| {
        let quantity = r.properties.get("quantity").map(display_value).unwrap_or_default();
        format!("{} {}", quantity, r.properties.get("name").map(display_value).unwrap_or_else(|| r.symbol.clone())).trim().to_owned()
    }).collect::<Vec<_>>();
    let instructions = recipe.operations.iter().map(|op| {
        let inputs = op.bindings.iter().filter(|b| b.role == BindingRole::Input).map(|b| b.resource.clone()).collect::<Vec<_>>();
        let detail = op.properties.get("description").map(display_value).unwrap_or_else(|| op.symbol.replace('_', " "));
    ...
    let ingredient_html = ingredients.iter().map(|x| format!("<li>{}</li>", escape(x))).collect::<String>();
    let instruction_html = instructions.iter().map(|x| format!("<li>{}</li>", escape(x))).collect::<String>();
    Ok(format!(r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><script type="application/ld+json">{json_ld}</script><style>{css}</style></head><body><main><header><p class="eyebrow">{site}</p><h1>{title}</h1><p>{description}</p></header><div class="grid"><article><section><h2>Ingredients</h2><ul>{ingredients}</ul></section><section><h2>Method</h2><ol>{instructions}</ol></section></article><aside>{label}</aside></div></main></body></html>"#,
        title=escape(&recipe.title), site=escape(options.site_title.as_deref().unwrap_or("Culinograph Recipe")),
        description=escape(options.description.as_deref().unwrap_or("")), ingredients=ingredient_html,
        instructions=instruction_html, label=label_svg, json_ld=json_ld, css=CSS))
}

fn display_value(value: &Value) -> String { match value { Value::Text(v)|Value::Symbol(v)=>v.clone(), Value::Number(v)=>v.to_string(), Value::Boolean(v)=>v.to_string(), Value::Quantity(q)=>format!("{} {}",q.value,q.unit), Value::List(v)=>v.iter().map(display_value).collect::<Vec<_>>().join(", "), Value::Object(_)=>String::new() } }
