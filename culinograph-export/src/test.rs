...
fn creates_zip_bundle() {
 let recipe=Recipe{id:Uuid::nil(),book_id:None,symbol:"tea".into(),declared_type:TypeRef::named("Recipe"),title:"Tea".into(),protocol_version:"0.3".into(),types:vec![],resources:vec![],processes:vec![],operations:vec![],servings:vec![],formulas:vec![],yields:vec![],properties:BTreeMap::new()};
 ...
 let bundle=StaticRecipeExporter.export(&recipe,"recipe tea {}",&options).unwrap();
 assert!(bundle.archive.starts_with(b"PK")); assert!(bundle.files.iter().any(|f|f.path=="index.html"));
}
