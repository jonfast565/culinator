...
impl Options {
    fn parse(args: impl Iterator<Item = String>) -> Result<Self, String> {
        let mut options = Self {
            database: PathBuf::from("culinograph-dev.sqlite3"),
            port: 0,
            token: None,
            origins: Vec::new(),
        ...
        while let Some(argument) = args.next() {
            match argument.as_str() {
                ...
                "--help" | "-h" => {
                    return Err("usage: culinograph-service [--db PATH] [--port PORT] [--token TOKEN] [--origin ORIGIN] [--allow-missing-origin]".to_owned());
                }
