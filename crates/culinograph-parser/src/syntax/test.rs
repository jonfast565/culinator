...
fn round_trip_preserves_comments_whitespace_and_unknown_tokens() {
    ...
    assert!(document.tokens().iter().any(|token| token.kind == SyntaxKind::BlockComment));
    assert!(document.tokens().iter().any(|token| token.kind == SyntaxKind::Unknown));
}
...
fn edits_preserve_untouched_bytes() {
    ...
    let edited = apply_text_edits(source, &[TextEdit::replace(TextRange::new(start, start + 3), "New")]).unwrap();
    assert_eq!(edited, "recipe x { title \"New\"; // keep\n}");
}

#[test]
fn rejects_overlapping_edits() {
    let edits = [TextEdit::delete(TextRange::new(0, 2)), TextEdit::delete(TextRange::new(1, 3))];
    assert!(matches!(apply_text_edits("abcd", &edits), Err(SyntaxError::OverlappingEdits(1))));
}
