...
pub fn init_database(database: &Path) -> Result<()> {
    ...
    culinator_sqlite::migrate(&connection)?;
...
pub fn import_recipe(file: &Path, database: &Path) -> Result<()> {
    ...
    let mut connection =
        Connection::open(database).with_context(|| format!("opening {}", database.display()))?;
    culinator_sqlite::migrate(&connection)?;
    culinator_sqlite::save_recipe(&mut connection, &recipe, &source)?;
...
pub fn import_recipe_book(file: &Path, database: &Path) -> Result<()> {
    let source = fs::read_to_string(file).with_context(|| format!("reading {}", file.display()))?;
    let book = culinator_parser::parse_recipe_book(&source)?;
    let mut connection =
        Connection::open(database).with_context(|| format!("opening {}", database.display()))?;
    culinator_sqlite::migrate(&connection)?;
    culinator_sqlite::save_recipe_book(&mut connection, &book)?;
    println!(
        "imported recipe book {} ({} recipes)",
        book.title,
        book.recipes.len()
    );
    Ok(())
...
pub fn list_books(database: &Path) -> Result<()> {
    let connection =
        Connection::open(database).with_context(|| format!("opening {}", database.display()))?;
    culinator_sqlite::migrate(&connection)?;
    for book in culinator_sqlite::list_recipe_books(&connection)? {
