...
The backend first attempts local OCR through the configured Tesseract executable. If OCR is unavailable or disabled, the OpenAI interpreter receives the images directly. The interpreter returns Culinograph 0.3 DSL, which is parsed and validated before being offered to the user.
