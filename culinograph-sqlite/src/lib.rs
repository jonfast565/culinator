...
fn replace_recipe_entities(tx: &Transaction<'_>, recipe: &Recipe) -> Result<()> {
    ...
    tx.execute(
        "DELETE FROM type_declarations WHERE recipe_id=?1",
        [&recipe_id],
    )?;
    ...
    for process in &recipe.processes {
        let parent_id = process
            .parent
            .as_ref()
            .and_then(|name| process_ids.get(name))
            .cloned();
        tx.execute(
    ...
    let operation_ids: HashMap<_, _> = recipe
        .operations
        .iter()
        .map(|operation| (operation.symbol.clone(), operation.id.to_string()))
        .collect();
    ...
    for operation in &recipe.operations {
        ...
        for dependency in &operation.dependencies {
            let Some(predecessor_id) = operation_ids.get(&dependency.predecessor) else {
                continue;
            };
            tx.execute(
        ...
        for binding in &operation.bindings {
            let Some(resource_id) = resource_ids.get(&binding.resource) else {
                continue;
            };
            tx.execute(
    ...
    for formula in &recipe.formulas {
        tx.execute(
            ...
            params![
                ...
                formula.symbol,
                formula.name,
                formula_basis_text(formula.basis),
                serde_json::to_string(&formula.properties).unwrap()
            ],
        )?;
        for (position, item) in formula.ingredients.iter().enumerate() {
            tx.execute(
                ...
                params![
                    ...
                    item.name,
                    item.stage,
                    formula_basis_text(item.basis),
                    item.percentage,
                    item.mass_grams,
                    item.is_flour as i64,
                    item.water_fraction,
                    position as i64,
                    serde_json::to_string(&item.properties).unwrap(),
                    item.is_reference as i64,
                    item.scalable as i64
                ],
            )?;
        }
    }
...
pub fn save_recipe_book(connection: &mut Connection, book: &RecipeBook) -> Result<()> {
    ...
    for (position, recipe) in book.recipes.iter().enumerate() {
        ...
        tx.execute(
            "UPDATE recipes SET book_position=?2 WHERE id=?1",
            params![recipe.id.to_string(), position as i64],
        )?;
        replace_recipe_entities(&tx, &recipe)?;
    }
    tx.commit()
...
pub fn move_recipe_to_book(
    ...
    Ok(connection.execute(
        "UPDATE recipes SET book_id=?2, book_position=?3, updated_at=CURRENT_TIMESTAMP WHERE id=?1",
        params![recipe_id, book_id, position],
    )? > 0)
}
...
pub fn save_formula(connection: &mut Connection, formula: &Formula) -> Result<()> {
    ...
    tx.execute(
        ...
        params![
            formula.id.to_string(),
            formula.recipe_id.map(|v| v.to_string()),
            formula.symbol,
            formula.name,
            formula_basis_text(formula.basis),
            serde_json::to_string(&formula.properties).unwrap()
    ...
    tx.execute(
        "DELETE FROM formula_ingredients WHERE formula_id=?1",
        [formula.id.to_string()],
    )?;
    for (position, item) in formula.ingredients.iter().enumerate() {
        tx.execute(
...
pub fn get_formula(connection: &Connection, formula_id: &str) -> Result<Option<Formula>> {
    let header = connection
        .query_row(
            "SELECT id, recipe_id, symbol, name, basis, properties_json FROM formulas WHERE id=?1",
            [formula_id],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
...
fn load_formula_parts(
    ...
    let ingredients = statement
        .query_map([&id], |row| {
            ...
            Ok(FormulaIngredient {
                ...
                name: row.get(2)?,
                stage: row.get(3)?,
                basis: parse_formula_basis(&basis_text),
                percentage: row.get(5)?,
                mass_grams: row.get(6)?,
                is_flour: row.get::<_, i64>(7)? != 0,
                water_fraction: row.get(8)?,
                is_reference: row.get::<_, i64>(10)? != 0,
                scalable: row.get::<_, i64>(11)? != 0,
    ...
    Ok(Formula {
        ...
        symbol,
        name,
        basis: parse_formula_basis(&basis),
        ingredients,
...
fn formula_basis_text(basis: FormulaBasis) -> &'static str {
    match basis {
        FormulaBasis::ReferencePercent => "bakers_percent",
...
fn parse_formula_basis(value: &str) -> FormulaBasis {
    match value {
        "percent_of_total" => FormulaBasis::PercentOfTotal,
        "absolute_mass" => FormulaBasis::AbsoluteMass,
        _ => FormulaBasis::ReferencePercent,
    }
}
