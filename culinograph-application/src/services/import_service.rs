        ...
        let validation = match self.parser.parse_recipe(&source_text) {
            Ok(recipe) => {
                ...
                let valid = !diagnostics
                    .iter()
                    .any(|d| d.severity == culinograph_models::DiagnosticSeverity::Error);
                culinograph_models::ValidationReport {
                    valid,
                    diagnostics,
                    outline: Some(culinograph_models::RecipeOutline {
            ...
            Err(error) => culinograph_models::ValidationReport {
                valid: false,
                diagnostics: vec![culinograph_models::SourceDiagnostic {
                    code: "IMPORT_PARSE".into(),
                    severity: culinograph_models::DiagnosticSeverity::Error,
